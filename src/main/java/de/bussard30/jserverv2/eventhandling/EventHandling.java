...
import de.bussard30.jserverv2.eventhandling.events.exceptions.NotRunningException;
...
public class EventHandling {
    ...
    public static void launch() {
        ...
        for (Map.Entry<Class<?>, Vector<Handler>> m : hm.entrySet()) {
            Collections.sort(m.getValue(), Comparator.comparingInt(Handler::getPriority));
        }
        launched = true;
    }
     ...
     */
    public static Event throwEvent(Event event, NetworkPhase n) throws NotRunningException {
        if (launched) {
            ...
            throw new NotRunningException("EventHandling has not been initialized or it is currently being halted!");
        }
        return event;
    }
    ...
    public static void handle(Packet p) {
        if (p instanceof Request) handleRequest((Request) p);
        else if (p instanceof Response) handleResponse((Response) p);
    }
